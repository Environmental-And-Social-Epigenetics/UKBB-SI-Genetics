#!/bin/bash
#SBATCH --job-name=mtag_convert_continuous
#SBATCH --partition=kellis
#SBATCH --mem=32G
#SBATCH -n 1
#SBATCH --time=1:00:00
#SBATCH --output=2_GWAS/logs/2b_mtag_convert.out
#SBATCH --error=2_GWAS/logs/2b_mtag_convert.err
...
echo "========================================"
echo "BOLT-LMM to MTAG Format Conversion (Continuous)"
...
echo "Running continuous MTAG conversion..."
echo ""

python3 ${SCRIPTDIR}/convert_to_MTAG_continuous.py
...
if [ ${convert_exit} -eq 0 ]; then
    ...
    echo ""
    echo "Next steps:"
    echo "1. Verify output files in mtag_results_continuous/<population>/"
    echo "2. Run MTAG analysis on these formatted files"
else
    echo "MTAG CONTINUOUS CONVERSION FAILED"
    echo "Exit code: ${convert_exit}"
    echo ""
    echo "Check error log: 2b_mtag_convert.err"
fi
echo ""
echo "End time: $(date)"
echo "========================================"
